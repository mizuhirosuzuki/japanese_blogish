...
例えば master do file のはじめに書いておいて、「このコードを回す人のPCにちゃんとietoolkitインストールされてますか？」とか「ちょっとバージョン古すぎますよ、アップデートしたほうがいいんじゃないですか？」みたいな確認ができます。
...
使い方としては、例えばHousehold IDが123456の家計のデータに問題があったとき、Aさんは「よし、これはデータから落とそう」ということで "drop if hhid == 123456" と書いたけど、Bさんが「これはIDを999999にしよう」ということで "replace hhid = 999999 if hhid == 123456" としてしまったとしましょう。
そうすると、Aさんのコードを回したときにStataは「ん、hhidが123456の人はいないね、じゃあ次いきまーす」とエラーメッセージを吐かずにずんずん進んでいってしまいます。
でも、 ”iedropone if hhid == 123456, numobs(1)" としておけば、「このコマンドで1つのサンプルが落とされないとエラーを出しますよ」ということになるので、hhid が 123456 の家計がいない今（つまりこのコマンドで落とされるサンプルの数は0）、プログラムはちゃんと止まってくれます。
ちょっと使い所はむずかしい気もするけど、フィールドワーク中とかには大事なのかもしれないですね。

## ieboilsave

このコマンドは、データを保存する前に「このデータ、ひとつひとつのサンプルにユニークなIDが与えられていますか？」というのをチェックしてくれます（他にも機能はあるけど、これが一番大事）。
例えば農村家計調査でプロット単位のデータだったら、「家計−プロット」ごとにユニークなIDを定義しましょう、ということです。
「家計IDとプロットIDでユニークに特定できるんだからいいじゃん」というのはベストプラクティスではないようです（mergeの時とかに困るからかな）。
...
missingok というオプションは、「本来は欠損値を単に "." とするんじゃなくて、欠損の理由に合わせて ".a", ".b", ... とするのがベストプラクティスだけど、まあそれは時間かかるし面倒なので、まあよしとしましょう」というものです。

# おわりに

今あるコマンドは以上ですが、DIME Analyticsでは今あるコマンドの改良＋新しいコマンドの開発を進めているようです。
というわけで、ちょっとでも役に立ったなーという方は[ietoolkitのGitHubのレポ](https://github.com/worldbank/ietoolkit)にいって星を押しましょう。
